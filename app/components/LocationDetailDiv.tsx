...
export default function LocationDetailDiv({ name, positionLeft = false}: LocationDetailDivProps) {
    ...
    return (
        <div className={`absolute text-white max-w-xs text-center ${name == null ? 'h-0 overflow-hidden' : 'h-auto border border-white border-2 p-4 shadow-lg '} bg-[#47bfbb] rounded-2xl ${positionLeft ? 'left-15' : 'right-15'} top-1/2 transform -translate-y-1/2 z-5`}>
